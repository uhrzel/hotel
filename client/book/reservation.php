        ...
        <div class="col-lg-12">
            <form role="form" id="booking" data-toggle="validator">
                ...
                <div class="col-lg-12">
                    ...
                    <button name="booking" type="submit" class="btn btn-lg btn-success pull-right" style="border-radius:0%">Submit</button>
                </div>
            </form>
        </div>
    </div>
        ...
        $("#booking").submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response === "success") {
                        // Update modal content with data from the server
                        $('#getCustomerName').text($('#first_name').val() + ' ' + $('#last_name').val());
                        $('#getRoomType').text($('#room_type option:selected').text());
                        $('#getRoomNo').text($('#room_no option:selected').text());
                        $('#getCheckIn').text($('#check_in_date').val());
                        $('#getCheckOut').text($('#check_out_date').val());
                        $('#getTotalPrice').text($('#total_price').text());
                        $('#getTotalPrice').text($('#total_price').text());
                        $('#getPaymentMode').text($('#payment_mode').text());
                        $('#getPaymentStaus').text('Pending');
